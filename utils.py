...
def search_pricing_table(param: str) -> str:
    """
    商品・料金テーブル検索に特化した Tool 設定用の関数。

    EcoTee の商品一覧・価格表（例: ./data/rag/pricing/商品情報.pdf）を参照して、
    ユーザーからの質問（param）に対して「資料に記載されている価格・料金プラン情報のみ」を返す。
    Agent から Tool として呼ばれることを想定しており、内部では pricing_doc_chain
    （料金テーブル専用の RAG チェーン）を実行して回答を生成する。

    期待される挙動の例:
        - 「ソフトタッチTシャツの価格は？」という入力に対し、
          商品情報.pdf に記載された価格（例: ¥2,700）をそのまま返す。
        - 複数候補がある場合は、候補の商品名と価格を列挙する。
        - 資料に価格が存在しない場合のみ、
          「この資料の中には該当商品の価格情報が記載されていません。」といった
          旨の回答を行う。

    この関数を利用することで、一般的な知識や推測ではなく、
    あくまで料金テーブル（商品情報.pdf など）の内容だけに基づいた
    一貫した価格回答を行うことを目的とする。

    Args:
        param: ユーザー入力値。
               例）「ソフトタッチTシャツの価格は？」「キッズ向けTシャツの価格帯は？」

    Returns:
        LLM からの回答テキスト（料金テーブルに基づく価格・プラン情報）。
    """
